...
from dygie.models.span_cls import SpanCls

logger = logging.getLogger(__name__)  # pylint: disable=invalid-name


@Model.register("cls_ner")
class ClsNER(Model):
    def __init__(self,
                 vocab: Vocabulary,
        ...
        super(ClsNER, self).__init__(vocab, regularizer)
        ...
        self._span = SpanCls.from_params(vocab=vocab, feature_size=feature_size, params=modules.pop("span"))
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        metrics_span = self._span.get_metrics(reset=reset)

        # Make sure that there aren't any conflicting names.
        metric_names = (list(metrics_ner.keys()) + list(metrics_span.keys())
                        )
        assert len(set(metric_names)) == len(metric_names)
        all_metrics = dict(
                           list(metrics_ner.items()) + list(metrics_span.items())
                           )
